...
🤓 Fun facts: 
- With exception to weekly 0.1% labs & "super assignments", I almost quit my first project & hence uni because of an ambiguous & arcane IDE bug generating erroneous results discovered not until the very final moments with just enough time to unfix countless code - "smells" - changes, when excluding all free time - basically zero. That is, when my eyes were not involuntarily closing from remote attacks (when 89% from completing examinations, for eg) & countles other such issues, including obscure co-requisite regulations initiating what is now infinite overtime (& would mention nocturnal home invasion attacks but I lost only sleep, which is nothing new nor old). It was a software engineering project for a Java GUI app simulating charges & quotes for multiple parking lot plans. It helped prepare me though for my next & first project in both Agile, git and game development while on my first formal exchange abroad.
